...
def seat_selection(request, flight_id):
    ...
    if request.method == 'POST':
        ...
        total_price = (
            flight.price * len(selected_seats)
        )
...
def delete_booking(request, booking_id):
    ...
    booking.flight.available_seats += (
    ...
    seat_numbers = booking.seat_number.split(",")
...
def dashboard(request):
    ...
    last_month_bookings = random.randint(
        80,
        200
    )

    last_month_profit = random.randint(
        200000,
        800000
    )

    growth_percentage = random.randint(
        10,
        40
    )
